...
//
// Same shape and contract as /api/top-stories but driven by a free-text query
// routed through TheNewsAPI's `search` parameter. Plain CommonJS, fully
// self-contained. See top-stories.js for the rationale on why this file is
// inlined instead of importing /api/_lib.
// =============================================================================
...
async function fetchSearchPool(query, category, limit) {
  ...
  if (!resp.ok) {
    ...
    console.error(`[search] TheNewsAPI ${resp.status}: ${body.slice(0, 200)}`);
...
module.exports = async function handler(req, res) {
  ...
  try {
    // Larger pool (40) since search results are sparser than the general
    // "today's news" stream. Up to 9 ranked stories — search is exploration,
    // not the daily brief.
    const pool = await fetchSearchPool(q, category, 40);
